...
CREATE TABLE locations (
  ...
  type TEXT NOT NULL CHECK (type IN ('warehouse', 'point_of_sale')),
  address TEXT,
...
CREATE TABLE inventory_batch_stocks (
  business_id TEXT NOT NULL,
...
-- Los movimientos anteriores dependían de dos saldos fijos dentro del lote. Al
-- reemplazarlos por ubicaciones explícitas se reconstruye el libro de movimientos.
DROP TABLE inventory_movements;

ALTER TABLE inventory_batches DROP COLUMN warehouse_quantity;
ALTER TABLE inventory_batches DROP COLUMN pos_quantity;
...
ALTER TABLE cash_sessions ADD COLUMN location_id TEXT REFERENCES locations(id);
ALTER TABLE sales ADD COLUMN location_id TEXT REFERENCES locations(id);

CREATE INDEX idx_locations_business ON locations(business_id, type, is_active, deleted_at);
CREATE INDEX idx_batch_stocks_location ON inventory_batch_stocks(business_id, location_id, quantity);
CREATE INDEX idx_batch_stocks_batch ON inventory_batch_stocks(business_id, batch_id);
