...
export function SuppliersPage() {
  ...
  async function submit(input: Omit<Supplier, "id">) {
    setError("");
    try {
      if (editing) await api.updateSupplier(editing.id, input);
    ...
    <section>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black">Proveedores</h1>
          <p className="mt-2 text-slate-500">
            Contactos y empresas que abastecen el inventario.
          </p>
        </div>
        <button
      ...
      <div className="mt-6 rounded-3xl bg-white shadow-sm">
                ...
                <tr key={supplier.id} className="border-t border-slate-100">
                  <td className="px-5 py-4">
                    ...
                    <p className="text-xs text-slate-400">
                      {supplier.taxId || "Sin identificación fiscal"}
                    </p>
                  </td>
                  <td>
                    <p className="font-bold">{supplier.contactName || "—"}</p>
                    <p className="text-xs text-slate-400">
        ...
        {visible.length === 0 && (
          <div className="grid place-items-center p-14 text-center text-slate-400">
            <Building2 size={40} />
            <p className="mt-3 font-bold">No hay proveedores para mostrar.</p>
          </div>
        )}
      </div>
      {editing !== undefined && (
        <div className="fixed inset-0 z-[60] grid place-items-center overflow-y-auto bg-slate-950/45 p-4">
          <FormProvider {...methods}>
            <form
              ...
              className="my-8 w-full max-w-2xl rounded-[2rem] bg-white p-7 shadow-2xl"
            >
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                ...
                <FieldInput
                  ...
                  register={methods.register("taxId")}
                />
                <FieldInput
                  label="Persona de contacto"
                  register={methods.register("contactName")}
                />
                <FieldInput
                  label="Teléfono"
                ...
                <FieldInput
                  label="País"
                  register={methods.register("country")}
                />
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button
                  ...
                  className="px-4 py-2 font-black text-slate-500"
                >
                  Cancelar
                </button>
                <button
