...
export function CategoriesPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(""); const data = new FormData(event.currentTarget); try { await api.createCategory(String(data.get("name"))); setOpen(false); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "No se pudo crear la categoría"); } }
  return <section><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-black uppercase tracking-wider text-emerald-700">Catálogo</p><h1 className="mt-1 text-3xl font-black">Categorías</h1><p className="mt-2 text-slate-500">Organiza los productos para encontrarlos rápidamente en el POS.</p></div><button onClick={() => { setError(""); setOpen(true); }} className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-5 py-3 font-black text-white"><Plus size={18} /> Nueva categoría</button></div>
    ...
    {open && <div className="fixed inset-0 z-[60] grid place-items-center bg-slate-950/45 p-4"><form onSubmit={submit} className="w-full max-w-md rounded-[2rem] bg-white p-7"><div className="flex justify-between"><h2 className="text-2xl font-black">Nueva categoría</h2><button type="button" onClick={() => setOpen(false)}><X /></button></div><div className="mt-6"><Field label="Nombre" name="name" required /></div>{error && <p className="mt-4 rounded-2xl bg-red-50 p-3 text-sm font-bold text-red-700">{error}</p>}<div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setOpen(false)} className="px-4 font-black text-slate-500">Cancelar</button><button className="rounded-xl bg-emerald-700 px-5 py-2.5 font-black text-white">Guardar</button></div></form></div>}
  </section>;
}
