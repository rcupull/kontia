...
export function InventoryPage() {
  ...
  const methods = useForm<{ name: string; type: "basic" | "composite"; categoryId: string }>({ defaultValues: { name: "", type: "basic", categoryId: "" } });
  ...
  function openForm(product: Product | null) {
    setError(""); setImage(null); setPreview(product?.imageId ? `/media/${product.imageId}` : "");
    methods.reset({ name: product?.name ?? "", type: product?.type ?? "basic", categoryId: product?.categoryId ?? "" }); setEditing(product);
  }

  async function submit(values: { name: string; type: "basic" | "composite"; categoryId: string }) {
    setError("");
    try {
      let imageId = editing?.imageId ?? null;
      if (image) imageId = (await api.uploadImage(await optimizeProductImage(image))).id;
      const input = { name: values.name, description: "", categoryId: values.categoryId || null, imageId, type: values.type };
  ...
  return <section>
    ...
    {editing !== undefined && <div className="fixed inset-0 z-[60] grid place-items-center overflow-y-auto bg-slate-950/45 p-4"><FormProvider {...methods}><form onSubmit={methods.handleSubmit(submit)} className="my-8 w-full max-w-lg rounded-[2rem] bg-white p-7 shadow-2xl"><div className="flex justify-between"><div><h2 className="text-2xl font-black">{editing ? "Editar producto" : "Nuevo producto"}</h2><p className="mt-1 text-sm text-slate-500">Los precios y cantidades se registran desde Inventario.</p></div><button type="button" onClick={() => setEditing(undefined)}><X /></button></div>
      <div className="mt-6 grid gap-4"><FieldInput label="Nombre" register={methods.register("name", { required: "El nombre es obligatorio" })} error={methods.formState.errors.name} /><FieldSelect label="Tipo" options={[{ value: "basic", label: "Básico" }, { value: "composite", label: "Compuesto" }]} register={methods.register("type", { required: "Selecciona el tipo" })} error={methods.formState.errors.type} /><FieldSelect label="Categoría" placeholder="Selecciona la categoría" options={categories.map((category) => ({ value: category.id, label: category.name }))} register={methods.register("categoryId", { required: "Selecciona una categoría" })} error={methods.formState.errors.categoryId} />
      ...
      {error && <p className="mt-4 rounded-2xl bg-red-50 p-3 text-sm font-bold text-red-700">{error}</p>}<div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setEditing(undefined)} className="px-4 font-black text-slate-500">Cancelar</button><button disabled={methods.formState.isSubmitting} className="rounded-xl bg-emerald-700 px-5 py-2.5 font-black text-white">Guardar</button></div></form></FormProvider></div>}
  </section>;
}
