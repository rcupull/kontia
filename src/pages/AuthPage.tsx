...
type AuthValues = { bootstrapSecret: string; businessName: string; displayName: string; username: string; password: string };

export function AuthPage() {
  const { setupRequired, setUser } = useAuth();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const methods = useForm<AuthValues>({ defaultValues: { bootstrapSecret: "", businessName: "", displayName: "", username: "", password: "" } });
  ...
  async function submit(data: AuthValues) {
    ...
    try {
      const result = setupRequired
        ? await api.setup({
            bootstrapSecret: data.bootstrapSecret, businessName: data.businessName,
  ...
  return (
    <main className="grid min-h-screen bg-[#f5f7f4] lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <section className="flex items-center justify-center p-6">
        <FormProvider {...methods}><form onSubmit={methods.handleSubmit(submit)} className="w-full max-w-md rounded-[2rem] bg-white p-8 shadow-xl shadow-emerald-950/5">
          <div className="mb-8">
          ...
          <div className="grid gap-4">
            {setupRequired && <>
              <FieldInputPassword label="Código de configuración" register={methods.register("bootstrapSecret", { required: "El código es obligatorio" })} error={methods.formState.errors.bootstrapSecret} />
              <FieldInput label="Nombre del negocio" register={methods.register("businessName", { required: "El negocio es obligatorio", minLength: { value: 2, message: "Usa al menos 2 caracteres" } })} error={methods.formState.errors.businessName} />
              <FieldInput label="Tu nombre" register={methods.register("displayName", { required: "Tu nombre es obligatorio" })} error={methods.formState.errors.displayName} />
            </>}
            <FieldInput label="Usuario" autoComplete="username" register={methods.register("username", { required: "El usuario es obligatorio" })} error={methods.formState.errors.username} />
            <FieldInputPassword label="Contraseña" autoComplete={setupRequired ? "new-password" : "current-password"} register={methods.register("password", { required: "La contraseña es obligatoria", minLength: setupRequired ? { value: 10, message: "Usa al menos 10 caracteres" } : undefined })} error={methods.formState.errors.password} />
          </div>
