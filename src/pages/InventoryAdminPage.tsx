...
export function InventoryAdminPage() {
  ...
  type MovementValues = { productId: string; batchId: string; movementType: string; quantity: number; unitCost: number; cashPrice: number; cardPrice: number; supplierInvoiceId: string; notes: string };
  const methods = useForm<MovementValues>({ defaultValues: { productId: "", batchId: "", movementType: "transferToPos", quantity: 0, unitCost: 0, cashPrice: 0, cardPrice: 0, supplierInvoiceId: "", notes: "" } });
  ...
  async function submit(values: MovementValues) { setError("");
    try { await api.createInventoryMovement({ productId: values.productId, movementType: values.movementType, batchId: createsBatch ? undefined : values.batchId, quantity: values.quantity,
  ...
  return <section><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm font-black uppercase tracking-wider text-emerald-700">Operaciones</p><h1 className="mt-1 text-3xl font-black">Inventario</h1><p className="mt-2 text-slate-500">Lotes, precios, ubicaciones y movimientos auditables.</p></div><button onClick={() => { setError(""); methods.reset(); setOpen(true); }} className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-5 py-3 font-black text-white"><Plus size={18} /> Nuevo movimiento</button></div>
    ...
    {open && <div className="fixed inset-0 z-[60] overflow-y-auto bg-slate-950/45 p-4"><FormProvider {...methods}><form onSubmit={methods.handleSubmit(submit)} className="mx-auto my-8 w-full max-w-xl rounded-[2rem] bg-white p-7 shadow-2xl"><div className="flex justify-between"><div><h2 className="text-2xl font-black">Nuevo movimiento</h2><p className="mt-1 text-sm text-slate-500">Compras, producciones e inyecciones crean lotes nuevos.</p></div><button type="button" onClick={() => setOpen(false)}><X /></button></div><div className="mt-6 grid gap-4">
      ...
      <FieldSelect label="Tipo de movimiento" options={movementOptions.map(([value, title, description]) => ({ value, label: `${title} — ${description}` }))} register={methods.register("movementType", { required: "Selecciona un tipo", onChange: () => { methods.setValue("productId", ""); methods.setValue("batchId", ""); } })} error={methods.formState.errors.movementType} />
      {!createsBatch && <FieldSelect label="Lote" placeholder="Seleccionar lote" options={availableBatches.map((b) => ({ value: b.id, label: `Almacén ${b.warehouseQuantity} | POS ${b.posQuantity} | Costo ${money(b.unitCostCents)}` }))} register={methods.register("batchId", { required: "Selecciona un lote" })} error={methods.formState.errors.batchId} />}
