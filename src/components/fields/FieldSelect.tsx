...
export const FieldSelect = <O extends any = any>({
  label,
  register,
  error,
  className,
  ...
  const selectedOption = useMemo(
    () => options.find((o) => String(optionValue(o)) === String(selectedValue)),
    [options, selectedValue, optionValue],
  );
  ...
  const displayLabel = selectedOption
    ...
    : placeholder;
  const filteredOptions = getSearchFilter
    ? options.filter((o) => getSearchFilter(search, o))
    : options;
  return (
    <div
      className={cn(
        "flex w-full min-w-0 max-w-full flex-col items-start",
        className,
      )}
    >
      ...
      <Popover.Root open={open} onOpenChange={setOpen}>
        <Popover.Trigger asChild>
          <button
            ...
            className={cn(
              "w-full min-w-0 max-w-full rounded-2xl border px-4 py-3 text-left focus:outline-none focus:ring-4 focus:ring-emerald-600/10",
              ...
              <div className="border-b p-2">
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setSearch(value);
                    debouncer(() => onSearch?.(value), searchDebounceMs);
                  }}
