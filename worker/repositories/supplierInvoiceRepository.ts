...
export class SupplierInvoiceRepository {
  ...
  async list(businessId: string, search = "") {
    ...
    const result = await this.db.prepare(`SELECT i.id, i.supplier_id AS supplierId,
      ...
      .bind(businessId, term, term, term).all();
  ...
  async create(businessId: string, input: SupplierInvoiceInput) {
    ...
    await this.db.prepare(`INSERT INTO supplier_invoices
      (id, business_id, supplier_id, invoice_number, invoice_date, total_amount_cents, notes)
      VALUES (?, ?, ?, ?, ?, ?, NULLIF(?, ''))`)
      .bind(id, businessId, input.supplierId, input.invoiceNumber, input.invoiceDate,
