...
export class CatalogRepository {
  ...
  async updateProduct(
    businessId: string,
    id: string,
    input: Omit<CreateCatalogProduct, "businessId" | "userId">,
  ) {
    const result = await this.db
      .prepare(
        `UPDATE products SET category_id = ?, image_id = ?,
  ...
  async getBatchesWithStock(
    ...
    const result = await this.db
      .prepare(
        `SELECT b.id,s.quantity FROM inventory_batches b
      ...
      .bind(locationId, businessId, productId)
      .all<StockBatch>();
    return result.results;
  }
  ...
  async adjustWarehouseStock(input: {
    ...
    const batches = await this.getBatchesWithStock(
      input.businessId,
      input.productId,
      input.locationId,
    );
    ...
    if (input.quantityDelta > 0) {
      ...
      statements.push(
        this.db
          .prepare(
            `UPDATE inventory_batch_stocks SET quantity=quantity+?,updated_at=datetime('now') WHERE batch_id=? AND location_id=? AND business_id=?`,
          )
          .bind(
            input.quantityDelta,
            batch.id,
      ...
      for (const batch of batches) {
        ...
        statements.push(
          this.db
            .prepare(
              `UPDATE inventory_batch_stocks SET quantity=quantity-?,updated_at=datetime('now') WHERE batch_id=? AND location_id=? AND business_id=? AND quantity>=?`,
            )
            .bind(
              quantity,
              batch.id,
  ...
  private movementStatement(
    ...
    return this.db
      ...
      .bind(
        ...
        input.productId,
        batchId,
        movementType === "negativeAdjustment" ? input.locationId : null,
        movementType === "positiveAdjustment" ? input.locationId : null,
        movementType,
        quantity,
        input.reason,
        input.userId,
