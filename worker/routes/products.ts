...
productRoutes.get("/", async (c) => {
  const products = await new CatalogRepository(c.env.DB).listProducts(
...
productRoutes.put("/:id", zValidator("json", productInput), async (c) => {
  const user = c.get("sessionUser");
  const updated = await new CatalogRepository(c.env.DB).updateProduct(
    user.businessId,
    c.req.param("id"),
    c.req.valid("json"),
  );
  return updated
    ? c.json({ ok: true })
    : c.json({ error: "Producto no encontrado" }, 404);
});
...
productRoutes.patch(
  ...
  async (c) => {
    const updated = await new CatalogRepository(c.env.DB).setProductActive(
      c.get("sessionUser").businessId,
      c.req.param("id"),
...
productRoutes.post(
  ...
  async (c) => {
    const user = c.get("sessionUser");
    try {
      const adjusted = await new CatalogRepository(
        c.env.DB,
      ...
      if (!adjusted)
        return c.json({ error: "Producto o lote no encontrado" }, 404);
      return c.json({ ok: true });
    } catch (error) {
      if (
        error instanceof Error &&
        error.message === "INSUFFICIENT_WAREHOUSE_STOCK"
      ) {
        return c.json(
          { error: "No hay suficiente existencia en almacén" },
          409,
        );
      }
      throw error;
...
productRoutes.get("/:id/movements", async (c) => {
  ...
  const movements = await new CatalogRepository(c.env.DB).listMovements(
    businessId,
    c.req.param("id"),
  );
