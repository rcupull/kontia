...
export async function getImage(c: Context<{ Bindings: Bindings }>) {
  ...
  if (bytes.byteLength !== image.size_bytes)
    return c.json({ error: "La imagen almacenada no es válida" }, 500);
...
export async function uploadImage(
  ...
  if (!(file instanceof File))
    return c.json({ error: "Selecciona una imagen" }, 400);
  if (!allowedTypes.has(file.type))
    return c.json({ error: "Usa una imagen JPG, PNG, WebP o AVIF" }, 400);
  if (file.size > MAX_IMAGE_SIZE)
    return c.json(
      { error: "La imagen optimizada no puede superar 600 KB" },
      400,
    );
  const id = await new ImageRepository(c.env.DB).create(
